...
describe('parseCommand', () => {
  ...
  // =================================================================
  // Task 9.2: コマンド語彙拡張（Issue #9）
  // =================================================================
  describe('コマンド語彙拡張（Issue #9）', () => {
    it('「右側」を right に変換する', () => {
      expect(parseCommand('右側')).toBe('right');
    });

    it('「みぎがわ」を right に変換する', () => {
      expect(parseCommand('みぎがわ')).toBe('right');
    });

    it('「右チーム」を right に変換する', () => {
      expect(parseCommand('右チーム')).toBe('right');
    });

    it('「左側」を left に変換する', () => {
      expect(parseCommand('左側')).toBe('left');
    });

    it('「ひだりがわ」を left に変換する', () => {
      expect(parseCommand('ひだりがわ')).toBe('left');
    });

    it('「左チーム」を left に変換する', () => {
      expect(parseCommand('左チーム')).toBe('left');
    });

    it('「取り消し」を rollback に変換する', () => {
      expect(parseCommand('取り消し')).toBe('rollback');
    });

    it('「とりけし」を rollback に変換する', () => {
      expect(parseCommand('とりけし')).toBe('rollback');
    });
  });
...
describe('isWakeword', () => {
  ...
  // =================================================================
  // Task 9.2: ウェイクワード拡張（Issue #10）
  // =================================================================
  it('「ヘイスコア」を検知する', () => {
    expect(isWakeword('ヘイスコア')).toBe(true);
  });

  it('「へいすこあ」を検知する（ひらがな）', () => {
    expect(isWakeword('へいすこあ')).toBe(true);
  });

  it('「ヘイスコアボード」を検知する（長文）', () => {
    expect(isWakeword('ヘイスコアボード')).toBe(true);
  });
