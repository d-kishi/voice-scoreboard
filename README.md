...
- [ ] ウェイクワード方式による誤検知防止
  - ウェイクワード「スコア」で認識モード開始
  - 認識モードは3秒間有効（タイムアウトで自動終了）
  - 認識モード中のみコマンドを受け付け
- [ ] 音声コマンドによる得点操作
  - 「スコア」→「右」: 画面右側のチームに+1点
  - 「スコア」→「左」: 画面左側のチームに+1点
  - 「スコア」→「ロールバック」: 直前の操作を取り消し
  - 「スコア」→「リセット」: 0-0の状態に戻す
- [ ] 音声認識のオン/オフ切り替え

#### 音声読み上げ機能

- [ ] 音声入力による得点変更時のみ自動読み上げ
  - 例：「左15 右12」
  - タップ操作による得点変更時は読み上げしない
- [ ] 試合終了時のホイッスル音（5秒間）
- [ ] 読み上げのオン/オフ切り替え
...
2. IDLE状態で「スコア」を検出 → LISTENING状態へ遷移、効果音再生
3. LISTENING状態で3秒以内にコマンドを検出 → 対応する操作を実行
4. コマンド実行後 or 3秒タイムアウト → IDLE状態に戻る
...
ウェイクワード「スコア」で認識モード（3秒間）に入った後、以下のコマンドを発声します。
