...
- [x] 3. スコアボード画面の実装
- [x] 3.0 (M1) Android Development Build 環境構築と起動確認
  - expo-dev-client のインストールと ANDROID_HOME 環境変数の設定
  - expo prebuild で android/ ディレクトリを生成
  - Android エミュレータ（Pixel_7a）でアプリが起動・表示されることを確認
  - 横画面（ランドスケープ）モードとダークネイビー背景の適用を検証
  - _Milestone: M1（ビルド・起動確認）_

- [x] 3.0.1 ビルド実行環境の Windows 移行検証 → **断念**
  - Windows 側 JDK 17 + ANDROID_HOME のセットアップ → 完了
  - JS バンドルを WSL2 で事前作成し、Gradle ビルドのみ Windows で実行 → 失敗（SDK ツールが Linux バイナリ、node_modules 破損等）
  - **結論**: WSL2 と Windows で node_modules / SDK を共有するハイブリッドアプローチは環境破壊リスクが高く断念
  - **副産物**: react-native-worklets@0.7.4 を依存追加（react-native-css-interop が要求、クリーンインストールで顕在化）

- [x] 3.1 左右チームのスコア表示パネル
...
- [x] 3.4 (M2) スコアボード画面のエミュレータプレビュー検証
  - Task 3.1〜3.3 の全 UI をエミュレータ（Pixel_7a release ビルド）で動作確認
  - スコア表示・+1/-1 操作・ControlBar・リセットダイアログ・試合終了オーバーレイの検証
  - _Milestone: M2（スコアボード UI 検証）_
...
- [x] 4.2 (P) 画面スリープ防止の組み込み
  - expo-keep-awake をメイン画面に組み込み、フォアグラウンド中は画面常時点灯
  - テスト: useKeepAwake hook が正しく呼ばれることの確認
  - _Requirements: 7.1_

- [x] 4.3 (M3) 設定永続化のエミュレータ検証
  - ControlBar のトグルが設定ストアと接続され、ON/OFF 状態が UI に反映されることを確認
  - アプリ再起動後に設定が復元されることを確認（AsyncStorage 永続化の実機検証）
  - _Milestone: M3（設定管理検証）_

- [x] 5. 音声サービス層の実装
- [x] 5.1 (P) 音声認識サービスの実装
  - expo-speech-recognition のラッパーサービスを作成
  - ウェイクワード検出モード: continuous: true でセッション維持（再起動ループ不要）
...
- [x] 5.4 (M4) 音声サービスのエミュレータ検証
  - 音声読み上げ（expo-speech）が日本語 TTS で発話できることを確認（エミュレータ内蔵 TTS エンジン使用）
  - 効果音（expo-av）がホイッスル音を再生できることを確認（エミュレータ音声出力 → Windows オーディオ）
  - 音声認識（expo-speech-recognition）をエミュレータで試行する
    - エミュレータの仮想マイクが Windows オーディオ経由で利用可能な場合は認識テスト実施
    - マイク入力が不可・SpeechRecognizer が動作しない場合は、サービス初期化と権限要求フローの確認に留め、認識精度の検証は M6（実機）に委ねる
  - 問題発覚時は Task 6 以降に進む前にここで対処する（リスク軽減チェックポイント）
  - _Milestone: M4（音声サービス エミュレータ検証）_

- [x] 6. 音声状態マシンの実装
- [x] 6.1 K.I.T.T.スタイル音声状態マシンの構築
  ...
  - LISTENING状態での5秒カウントダウンとタイムアウトによるIDLE復帰
...
- [x] 7.4 (M5) 音声UI統合のエミュレータ検証
  - LISTENING オーバーレイの表示（シアンリング・カウントダウン・ブラー効果）を確認
  - ホイッスル音の試合終了連携を確認
  - 設定トグル（音声認識 ON/OFF、読み上げ ON/OFF）の反映を確認
  - 音声認識がエミュレータで動作する場合: E2E フロー（「スコア」→「Ready」→「右/左」→スコア反映）を確認
